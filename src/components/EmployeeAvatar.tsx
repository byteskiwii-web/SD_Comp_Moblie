 ...
 * refusal is a 404 and lands here as a failed image, which falls back to
 * initials -- the same thing it shows while there is no photo at all, which is
 * the honest outcome either way.
 *
 * NO CACHE-BUSTING VERSION. photoUpdatedAt is part of the signed-in profile
 * and is not known for a colleague, so a replaced picture can persist until
 * the cache expires. Worth it: the alternative is a request per row to learn
 * a timestamp that changes once a year.
 */
